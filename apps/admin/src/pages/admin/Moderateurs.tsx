...
import { CheckCircle2, XCircle, UserCog, Shield, AlertCircle, Clock, FileCheck } from 'lucide-react';
...
const ACCESS_ITEMS = [
  ...
  { label: 'Qualité — voir scores + alertes', ok: true },
  { label: 'Candidatures — approuver/rejeter', ok: true },
  { label: 'Réservations — lecture seule', ok: true },
  { label: 'Clients — données limitées', ok: true },
  { label: 'Finance', ok: false },
  { label: 'Paiements / Factures', ok: false },
  { label: 'Conversion prestataire directe', ok: false },
  { label: 'Suppression définitive', ok: false },
  { label: 'Paramètres / RGPD', ok: false },
];

const RULES = [
  { id: 'R-MO-01', text: 'Approbation document → pending_ao, validation finale par Agent Opérationnel' },
  { id: 'R-MO-02', text: 'Peut rejeter un avis avec motif mais pas le supprimer définitivement' },
  { id: 'R-MO-03', text: 'Chaque décision nécessite une note documentée (min 10 caractères)' },
  { id: 'R-MO-04', text: 'Signalement urgent → escalade automatique vers Super Admin' },
  { id: 'R-MO-05', text: 'Toutes les décisions loguées avec horodatage et motif' },
...
export default function Moderateurs() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await callMO('list_agents');
  ...
  const assign = useMutation({
    mutationFn: async (email: string) => {
      ...
      return callMO('assign', { userId: prof.user_id, charterAcknowledged: charterChecked });
    },
    onSuccess: () => {
      toast({ title: 'Rôle Modérateur assigné (expire dans 6 mois)' });
  ...
  const revoke = useMutation({
    mutationFn: (userId: string) =>
      callGovernance('revoke_with_email', { userId, role: 'moderator', reason: 'Révoqué depuis interface admin' }),
    onSuccess: () => {
      toast({ title: 'Rôle révoqué — email envoyé au modérateur' });
      setRevokeId(null);
      qc.invalidateQueries({ queryKey: ['moderateur-agents'] });
    },
    onError: (e: Error) => toast({ title: e.message, variant: 'destructive' }),
  ...
  const renew = useMutation({
    mutationFn: (userId: string) =>
      callGovernance('renew', { userId, role: 'moderator', months: 6 }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold">Modérateurs</h1>
          <p className="text-muted-foreground">Rôle de modération du contenu — R-MO-01 à R-MO-05</p>
        </div>
        <Badge className="ml-auto" variant="secondary">
          {agents.length} modérateur{agents.length !== 1 ? 's' : ''}
        </Badge>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader><CardTitle>Modérateurs actifs</CardTitle></CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="flex justify-center py-8"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary" /></div>
            ) : agents.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-6">Aucun modérateur assigné.</p>
            ) : (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-sm"><UserCog className="w-4 h-4" />Assigner le rôle MO</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-xs text-muted-foreground">L'utilisateur devra se reconnecter. Décisions loguées (R-MO-05).</p>
            <Input
            ...
            <div className="flex items-start gap-2 p-2 rounded border border-amber-200 bg-amber-50 dark:bg-amber-950 dark:border-amber-800">
              <Checkbox id="charter-mo" checked={charterChecked} onCheckedChange={v => setCharterChecked(v === true)} className="mt-0.5" />
              <label htmlFor="charter-mo" className="text-xs text-amber-700 dark:text-amber-300 cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Shield className="w-5 h-5 text-purple-500" />Droits d'accès</CardTitle></CardHeader>
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><AlertCircle className="w-5 h-5 text-orange-500" />Règles R-MO</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {RULES.map(rule => (
