...
import {
  ...
  Calendar, Clock, Euro, AlertTriangle, CheckCircle, Loader2
} from 'lucide-react';
...
const QUERY_KEY = (status: string, search: string) =>
  ['admin-custom-requests', status, search] as const;
...
export const AdminCustomRequests = () => {
  ...
  const { data: requests = [], isFetching: loading, refetch } = useQuery<CustomRequest[]>({
    queryKey: QUERY_KEY(statusFilter, searchTerm),
    queryFn: async () => {
      let query = supabase
      ...
      if (searchTerm.trim()) {
        query = query.or(
          `client_name.ilike.%${sanitizeSearch(searchTerm)}%,client_email.ilike.%${sanitizeSearch(searchTerm)}%,service_description.ilike.%${sanitizeSearch(searchTerm)}%,location.ilike.%${sanitizeSearch(searchTerm)}%`
        );
      }
    ...
    try {
      ...
      supabase.functions.invoke('send-modern-notification', {
        body: {
          ...
          recipient: { email: selected.client_email, name: selected.client_name, firstName: selected.client_name.split(' ')[0] },
          data: { clientName: selected.client_name, newStatus, serviceDescription: selected.service_description, adminNote: adminNote || undefined, bookingId: selected.id },
        },
      }).catch(() => {});
  ...
  const openDetail = (req: CustomRequest) => { setSelected(req); setNewStatus(req.status); setAdminNote(''); };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Demandes soumises via le formulaire client</p>
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-16 text-muted-foreground">
              <MessageSquare className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Aucune demande trouvée</p>
            </div>
          ) : (
            <div className="divide-y">
              {requests.map(req => {
                ...
                return (
                  <div key={req.id} className="p-4 hover:bg-muted/40 transition-colors cursor-pointer" onClick={() => openDetail(req)}>
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {req.urgency_level && req.urgency_level !== 'normal' && (
                            <span className={`text-xs font-medium ${urgencyCfg.color}`}>⚡ {urgencyCfg.label}</span>
                          )}
                        </div>
                        <p className="text-sm text-muted-foreground line-clamp-2">{req.service_description}</p>
                        <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                          <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{req.client_email}</span>
                          {req.client_phone && <span className="flex items-center gap-1"><Phone className="w-3 h-3" />{req.client_phone}</span>}
                      ...
                      <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-1 text-xs text-muted-foreground flex-shrink-0">
                        <span className="flex items-center gap-1">
                          <Calendar className="w-3 h-3" />{format(new Date(req.created_at), 'dd/MM/yyyy', { locale: fr })}
                        </span>
                        {req.budget_range && (
                          <span className="flex items-center gap-1 font-medium text-foreground"><Euro className="w-3 h-3" />{req.budget_range}</span>
                        )}
              ...
              <div className="space-y-4 mt-2">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    {selected.budget_range && (
                      <p className="text-sm font-medium flex items-center gap-1"><Euro className="w-3 h-3" />Budget : {selected.budget_range}</p>
                    )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm space-y-1">
                    <p className="flex items-start gap-1.5"><MapPin className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" /><span><strong>Lieu :</strong> {selected.location}</span></p>
                    {selected.pickup_address && <p className="flex items-start gap-1.5 ml-5 text-muted-foreground">Départ : {selected.pickup_address}</p>}
                    {selected.delivery_address && <p className="flex items-start gap-1.5 ml-5 text-muted-foreground">Livraison : {selected.delivery_address}</p>}
                  </div>
                </div>
                {(selected.preferred_date || selected.preferred_time) && (
                  <div className="flex items-center gap-4 text-sm">
                    {selected.preferred_date && <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-primary" />{format(new Date(selected.preferred_date), 'dd MMMM yyyy', { locale: fr })}</span>}
                    {selected.preferred_time && <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-primary" />{selected.preferred_time}</span>}
                  </div>
                )}
                {selected.additional_notes && (
                  <div className="space-y-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <Textarea value={adminNote} onChange={e => setAdminNote(e.target.value)} placeholder="Ajoutez une note interne…" rows={2} className="resize-none text-sm" />
                    </div>
                  </div>
                  <div className="flex gap-2 justify-end">
                    <Button variant="outline" onClick={() => { setSelected(null); setAdminNote(''); setNewStatus(''); }}>Fermer</Button>
                    <Button onClick={updateStatus} disabled={updating || (newStatus === selected.status && !adminNote)}>
