...
interface Props {
  children: ReactNode;
  /** Rôles bloqués (les admins passent toujours, quel que soit ce tableau) */
  deny: string[];
}

export function RoleGatedRoute({ children, deny }: Props) {
  const { hasRole } = useAuth();

  const isBlocked = !hasRole('admin') && deny.some(role => hasRole(role as any));
      ...
      <div className="flex items-center justify-center min-h-[400px] p-6">
        <Card className="w-full max-w-md border-destructive/50">
          ...
          <CardContent className="text-center text-sm text-muted-foreground space-y-2">
            <p>Cette section n'est pas accessible avec votre rôle actuel.</p>
