...
async function fetchAnalytics() {
  ...
  const bookingsByDay = last7Days.map(date => ({
    ...
    reservations: bookingsData?.filter(b => b.created_at?.startsWith(date)).length || 0,
  }));
  ...
  return {
    ...
    bookingsByDay,
    revenueByCategory,
    providerPerformance: topProviders || [],
  };
}
...
export const AdvancedAnalytics = () => {
  ...
  const metrics           = data?.metrics           ?? { totalBookings: 0, totalRevenue: 0, activeUsers: 0, avgRating: 0, growthRate: 12.5 };
  ...
  if (loading) return <div className="p-8">Chargement des analytics...</div>;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold mb-2">Analytics Avancés</h1>
        <p className="text-muted-foreground">Métriques détaillées et indicateurs de performance</p>
      </div>

      <div className="grid gap-4 md:grid-cols-5">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium flex items-center gap-2"><Calendar className="w-4 h-4" />Réservations</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium flex items-center gap-2"><DollarSign className="w-4 h-4" />Revenu Total</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium flex items-center gap-2"><Users className="w-4 h-4" />Utilisateurs Actifs</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium flex items-center gap-2"><Star className="w-4 h-4" />Note Moyenne</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium flex items-center gap-2"><TrendingUp className="w-4 h-4" />Croissance</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Tabs defaultValue="bookings" className="space-y-4">
        ...
        <TabsContent value="bookings" className="space-y-4">
          <Card>
            <CardHeader><CardTitle>Évolution des réservations (7 derniers jours)</CardTitle></CardHeader>
            <CardContent>
        ...
        <TabsContent value="revenue" className="space-y-4">
          <Card>
            <CardHeader><CardTitle>Répartition du revenu par catégorie</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie data={revenueByCategory} cx="50%" cy="50%" labelLine={false}
        ...
        <TabsContent value="providers" className="space-y-4">
          <Card>
            <CardHeader><CardTitle>Top 5 Prestataires</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={providerPerformance}>
                  <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="business_name" /><YAxis />
                  <Tooltip /><Legend />
