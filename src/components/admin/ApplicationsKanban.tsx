...
const QUERY_KEY = ['admin-kanban-applications'] as const;

const columns: Column[] = [
  { id: 'pending',           title: 'Nouvelles',    color: 'text-blue-600',   bgColor: 'bg-blue-50 dark:bg-blue-950/30',     icon: <Clock className="w-4 h-4" /> },
  { id: 'reviewing',         title: 'En révision',  color: 'text-amber-600',  bgColor: 'bg-amber-50 dark:bg-amber-950/30',   icon: <Eye className="w-4 h-4" /> },
  { id: 'documents_pending', title: 'Documents',    color: 'text-purple-600', bgColor: 'bg-purple-50 dark:bg-purple-950/30', icon: <FileText className="w-4 h-4" /> },
  { id: 'approved',          title: 'Approuvées',   color: 'text-emerald-600',bgColor: 'bg-emerald-50 dark:bg-emerald-950/30',icon: <CheckCircle className="w-4 h-4" /> },
  { id: 'rejected',          title: 'Refusées',     color: 'text-red-600',    bgColor: 'bg-red-50 dark:bg-red-950/30',       icon: <XCircle className="w-4 h-4" /> },
];
...
export const ApplicationsKanban = () => {
  ...
  const { data: applications = [], isLoading: loading, refetch } = useQuery<Application[]>({
    queryKey: QUERY_KEY,
    queryFn: async () => {
    ...
    applications
      ...
      .filter(app => {
        ...
        return (
          ...
          app.category.toLowerCase().includes(s) ||
        ...
        {columns.map(column => (
          <Card key={column.id} className={cn("border-0 shadow-sm", column.bgColor)}>
            <CardContent className="p-3 flex items-center justify-between">
              ...
              <Badge variant="secondary" className="text-sm">{getApplicationsByStatus(column.id).length}</Badge>
            </CardContent>
          </Card>
        ))}
      </div>
          ...
          {columns.map(column => (
            <div key={column.id} className="w-full sm:w-[calc(50%-0.5rem)] lg:w-auto lg:min-w-[260px] lg:flex-1">
              <div className={cn("rounded-t-lg p-3 flex items-center gap-2", column.bgColor)}>
                ...
                <Badge variant="outline" className="ml-auto text-xs">{getApplicationsByStatus(column.id).length}</Badge>
              </div>
                          ...
                          >
                            <CardContent className="p-3 space-y-2">
                              ...
                              <div className="space-y-1 text-xs text-muted-foreground">
                                <div className="flex items-center gap-1.5"><Mail className="w-3 h-3" /><span className="truncate">{app.email}</span></div>
                                {app.city && <div className="flex items-center gap-1.5"><MapPin className="w-3 h-3" /><span>{app.city}</span></div>}
                              ...
                              {app.experience_years && (
                                <div className="pt-1 border-t">
                                  <span className="text-xs text-muted-foreground">{app.experience_years} ans d'expérience</span>
                                </div>
                              )}
                            </CardContent>
                          </Card>
