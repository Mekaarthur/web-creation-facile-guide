...
const FinancialRulesManager = () => {
  ...
  const fetchRules = async () => {
    try {
      ...
      toast({ title: "Erreur", description: "Impossible de charger les règles financières", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggle = async (id: string, isActive: boolean) => {
    ...
    if (!error) { toast({ title: isActive ? "Activé" : "Désactivé" }); fetchRules(); }
  };
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="space-y-2">
        {rules.map((rule) => {
          ...
          const marginPct = rule.client_price > 0
            ? Math.round((margin / rule.client_price) * 100)
            : 0;
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-[1fr_110px_110px_80px_90px_40px] gap-3 items-center">
                  ...
                  <div className="text-center">
                    {isEditing ? (
                      ...
                      <div className="flex items-center justify-center gap-1 font-semibold text-green-700">
                        ...
                        {rule.provider_payment.toFixed(2)} €
                      </div>
                    )}
                    ...
                    {isEditing ? (
                      <div className="text-xs text-muted-foreground text-center">
                        <p className="font-medium text-foreground">
                          {(editValues.client_price - editValues.provider_payment).toFixed(2)} €
                        </p>
                        <p>
                          {editValues.client_price > 0
                            ? Math.round(((editValues.client_price - editValues.provider_payment) / editValues.client_price) * 100)
                            : 0} %
                        </p>
                      </div>
                    ) : (
                  ...
                  <div className="flex justify-end gap-1">
                    {isEditing ? (
                      <>
                        <Button size="sm" className="h-8 w-8 p-0" onClick={() => handleSave(rule.id)}>
                          <Save className="w-3.5 h-3.5" />
                        </Button>
                        <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => setEditingId(null)}>
