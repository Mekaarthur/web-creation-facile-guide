          ...
          <div className={`space-y-8 text-center lg:text-left ${isInView ? 'animate-fade-in-up' : 'opacity-0'}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <Link to="/services" className="w-full sm:w-auto">
                <Button 
                  size="xl" 
                  className="group w-full sm:w-auto text-lg px-8 py-4 bg-gradient-hero hover:scale-105 transition-all duration-300 shadow-glow touch-target"
                >
                  <Zap className="w-6 h-6 mr-3" />
                  Déléguer mes missions
            ...
            <div className="flex items-center justify-center lg:justify-start space-x-6 pt-4">
              <div className="flex -space-x-3">
                {[1,2,3,4,5].map((i) => (
                  <div key={i} className="w-12 h-12 bg-gradient-primary rounded-full border-3 border-white shadow-soft"></div>
                ))}
              </div>
              <div className="text-left">
                <div className="text-2xl font-bold text-foreground">2,500+</div>
                <div className="text-sm text-muted-foreground">familles sereines</div>
              </div>
            </div>
          </div>
