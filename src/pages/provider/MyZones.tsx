...
async function fetchProviderZones(userId: string): Promise<ZonesData> {
  ...
  if (!providerData) throw new Error('Profil prestataire non trouvé');
  ...
  return { providerId: providerData.id, zones };
}
...
const MyZones = () => {
  ...
  const { data, isLoading: loading } = useQuery<ZonesData>({
    queryKey: ['provider-zones', user?.id],
    queryFn: () => fetchProviderZones(user!.id),
    enabled: !!user?.id,
  });
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['provider-zones'] });
  ...
  const handleSaveZone = async () => {
    ...
    if (!formData.adresse_reference.trim()) {
      toast({ title: "Erreur", description: "Veuillez saisir une adresse", variant: "destructive" });
      return;
    }
    try {
      const zoneData: any = {
      ...
      if (editingZone) {
        const { error } = await supabase.from('prestataire_zones').update(zoneData).eq('id', editingZone.id);
        if (error) throw error;
        toast({ title: "Zone modifiée", description: "Votre zone d'intervention a été mise à jour" });
      } else {
        const { error } = await supabase.from('prestataire_zones').insert([zoneData]);
        if (error) {
          if (error.message.includes('Limite de zones atteinte')) {
            toast({ title: "Limite atteinte", description: "Vous avez atteint le nombre maximum de zones actives (5)", variant: "destructive" });
            return;
          }
          throw error;
        }
        toast({ title: "Zone ajoutée", description: "Votre nouvelle zone d'intervention a été créée" });
      }
  ...
  const handleDelete = async (zoneId: string, adresse: string) => {
    ...
    try {
      const { error } = await supabase.from('prestataire_zones').delete().eq('id', zoneId);
      if (error) throw error;
      toast({ title: "Zone supprimée", description: "La zone d'intervention a été supprimée" });
      invalidate();
    } catch {
  ...
  const toggleZoneStatus = async (zoneId: string, currentStatus: string) => {
    ...
    try {
      const { error } = await supabase.from('prestataire_zones').update({ statut: newStatus }).eq('id', zoneId);
      if (error) throw error;
      toast({
  ...
  const updateDisponibilite = (day: string, value: boolean) => {
    setFormData({ ...formData, disponibilite: { ...formData.disponibilite, [day]: value } });
  };
    ...
    <div className="space-y-6 p-6">
        ...
        <Card className="border-l-4 border-l-blue-500 bg-blue-50/50">
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-blue-900 mb-1">Définissez vos zones d'intervention</h3>
        ...
        <Card className="border-l-4 border-l-orange-500 bg-orange-50/50">
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-orange-900 mb-1">Limite de zones atteinte</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Toutes zones confondues</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Total par zone</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Satisfaction clients</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Voici les zones où vous êtes actuellement disponible</CardDescription>
        </CardHeader>
        <CardContent>
          {zones.length === 0 ? (
                ...
                {zones.map((zone) => (
                  <TableRow key={zone.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">{zone.type_zone}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-center">
                      ...
                      <div className="text-xs text-muted-foreground">{zone.missions_accepted || 0} acceptées</div>
                    </TableCell>
                    <TableCell className="text-center">
                    ...
                    <TableCell>
                      <Badge variant={zone.statut === 'active' ? "default" : zone.statut === 'paused' ? "secondary" : "destructive"}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button variant="ghost" size="sm" onClick={() => toggleZoneStatus(zone.id, zone.statut)}>
                          {zone.statut === 'active' ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(zone)}>
                          <Edit2 className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(zone.id, zone.adresse_reference)}>
