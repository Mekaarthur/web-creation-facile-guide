...
const providerPrice = (clientPrice: number | string) =>
  typeof clientPrice === "number"
    ? `${calculateProviderPrice(clientPrice).toFixed(2)} €`
    : "Sur devis";
...
const AdminPricing = () => {
  ...
  // Prix en base (override DB)
  const [dbPrices, setDbPrices] = useState<Record<string, DbPrice>>({});
  ...
  const loadPrices = useCallback(async () => {
    ...
    try {
      ...
      (data || []).forEach((row: any) => {
        map[row.service_slug] = row;
      });
      setDbPrices(map);
    } catch (err) {
      console.error("Erreur chargement prix:", err);
      toast({ title: "Erreur", description: "Impossible de charger les prix", variant: "destructive" });
    } finally {
  ...
  // -----------------------------------------------------------------------
  // Ouvrir la modale de confirmation d'édition
  // -----------------------------------------------------------------------
  const openEdit = (
    slug: string,
    universeId: string,
  ...
  const savePrice = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${editState.serviceName} : ${newPrice.toFixed(2)} €/h`,
      });
      setEditState(null);
      await loadPrices();
    } catch (err: any) {
  ...
  // -----------------------------------------------------------------------
  // Prix effectif (DB si dispo, sinon statique)
  // -----------------------------------------------------------------------
  const effectivePrice = (slug: string, staticPrice: number | string) =>
    dbPrices[slug]?.client_price ?? (typeof staticPrice === "number" ? staticPrice : null);
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Button variant="outline" size="sm" onClick={loadPrices} disabled={loadingPrices}>
      ...
      <Tabs defaultValue="prices">
        <TabsList>
          ...
          <TabsTrigger value="history" onClick={loadAudit}>
                ...
                <div className="space-y-2">
                  ...
                  {auditLogs.map((log) => (
                    ...
                    >
                      ...
                      <div className="text-center text-sm line-through text-muted-foreground">
                      ...
                      <div className="text-center">
                        {diffBadge(log.old_price, log.new_price)}
                      </div>
                      <div className="text-sm text-muted-foreground">{log.admin_name}</div>
                      <div className="text-xs text-muted-foreground">
                        {new Date(log.changed_at).toLocaleDateString("fr-FR", {
                          day: "numeric", month: "short", year: "numeric",
                        })}
                        <br />
                        {new Date(log.changed_at).toLocaleTimeString("fr-FR", {
                ...
                <div className="text-xs text-muted-foreground bg-muted/20 rounded-lg p-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {calculateProviderPrice(parseFloat(editState.newPrice)).toFixed(2)} €
                    </span>
                  </div>
