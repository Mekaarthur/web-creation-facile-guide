-- Créer la table zones_geographiques si elle n'existe pas déjà
...
-- Enable RLS sur zones_geographiques si pas déjà fait
...
DROP POLICY IF EXISTS "Admin peut gérer les zones" ON public.zones_geographiques;
...
DROP TRIGGER IF EXISTS update_zones_geographiques_updated_at ON public.zones_geographiques;
...
-- Insérer quelques zones par défaut (Île-de-France) seulement si la table est vide
INSERT INTO public.zones_geographiques (nom_zone, codes_postaux, type_zone) 
SELECT nom_zone, codes_postaux, type_zone
FROM (VALUES
...
) AS v(nom_zone, codes_postaux, type_zone)
WHERE NOT EXISTS (SELECT 1 FROM public.zones_geographiques LIMIT 1);