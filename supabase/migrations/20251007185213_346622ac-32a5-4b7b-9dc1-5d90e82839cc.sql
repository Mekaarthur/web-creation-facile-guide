...
DROP POLICY IF EXISTS "Anonymous users can create limited conversations" ON public.chatbot_conversations;

-- =============================================================================
-- 2. Create secure INSERT policies with validation
-- =============================================================================

-- For authenticated users
CREATE POLICY "Secure authenticated conversation creation"
...
WITH CHECK (
  ...
  -- Must provide at least email or link to user_id
  AND (user_id IS NOT NULL OR user_email IS NOT NULL)
);

-- For anonymous users with strict limitations
CREATE POLICY "Secure anonymous conversation creation"
ON public.chatbot_conversations
FOR INSERT
TO anon
WITH CHECK (
