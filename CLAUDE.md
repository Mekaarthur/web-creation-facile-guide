...
- **R-SEL-12** : Avant `create-payment`, `BookingCheckout.runPreflightChecks()` vérifie : montant > 0, date >= J+1, adresse renseignée, prestataire disponible dans la zone (`bookingService.hasAvailableProviderInZone`), pas de double réservation client sur le créneau (`bookingService.hasConflictingBooking`, cf. R7), panier non expiré (< 30 min via `bikawo-cart-timestamp`). Échec → message précis en toast, client reste sur la page, aucune navigation.
